<!-- src/routes/admin/organizations/OrganizationList.svelte -->
<script>
  ...
  /**
   * Handle selecting a role for an organization
   * @param {Object} role - The practitioner role to select
   */
  async function handleSelectRole(role) {
    isSelectingRole = true;
    selectedOrganizationId = role?.organization?.id || null;
    try {
...
<div class="organization-list">
  ...
  <ul>
    {#each organizations as org}
      <li>
        <button 
          on:click={() => handleSelectRole(
            practitionerRoles.find(role => role.organization?.id === org.id)
          )}
          disabled={
            isSelectingRole || 
            !practitionerRoles.some(role => role.organization?.id === org.id)
          }
          aria-label={`Select role for organization ${org.name}`}
        >
          {isSelectingRole && selectedOrganizationId === org.id 
            ? 'Selecting...' 
            : org.name}
        </button>
      </li>
